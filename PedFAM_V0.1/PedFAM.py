...
if __name__ == '__main__':
    ...
    for i in range(num_Chro):
        Prob_No_Switch_Happen, Prob_Switch_Happen, Prob_Ref_f, Prob_Ref_m = PreProcess('AF_C'+str(i+1)+'.dat', 'test_1_C'+str(i+1)+'.dat', 'test_2_C'+str(i+1)+'.dat', 'ref_position_C'+str(i+1)+'.dat', Num_Block, recombination_r, num_ref)
    ...
    print(founders_Opt4, Prob_Opt4)
    if(Prob_Opt4 > Prob_Opt):
        Prob_Opt = Prob_Opt4
        founders_Opt = founders_Opt4[:]

    print('final optimal:', founders_Opt, Prob_Opt)
